/*
----------------
-- apl102_validation_check_total_nw_04
----------------
*/
/**
 * run.
...
let utils = {
  ...
  countBytes: function (text) {
    return utils.isEmpty(text) ? 0 : unescape(encodeURIComponent(text)).length;
  },

  isJapanese: function (text) {
    return /^([\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\uff66-\uff9f\uff10-\uff19])+$/.test(
      text
    );
  },
  ...
  validateNumber: function (numberString) {
  ...
  groupByError: function (column, group, value) {
    group.error = "true";
    if (!group[column]) {
      group[column] = [];
    }
